...
class GestorSeguimiento
{
    ...
    public function alternarSeguimiento($id_usuario, $id_artista)
    {
        ...
        $db = $conObj->conectar();
        if ($db === null)
            return ['status' => 'error', 'message' => 'Error BD'];

        try {
            // Verificar si ya lo sigue
            $sqlCheck = "SELECT COUNT(*) as total FROM Seguimiento_Artista WHERE FK_id_usuario = ? AND FK_id_artista = ? AND activo = 1";
            $stmtCheck = $db->prepare($sqlCheck);
            $stmtCheck->execute([$id_usuario, $id_artista]);
            $sigue = $stmtCheck->fetch(PDO::FETCH_ASSOC)['total'] > 0;

            if ($sigue) {
                // Unfollow: borrado lógico con UPDATE activo=0 (Regla Oro #4)
                $sqlDel = "UPDATE Seguimiento_Artista SET activo = 0 WHERE FK_id_usuario = ? AND FK_id_artista = ?";
    ...
    public function verificarSeguimiento($id_usuario, $id_artista)
    {
        ...
        try {
            $sql = "SELECT COUNT(*) as total FROM Seguimiento_Artista WHERE FK_id_usuario = ? AND FK_id_artista = ? AND activo = 1";
            $stmt = $db->prepare($sql);
    ...
    public function obtenerArtistasSeguidos($id_usuario)
    {
        ...
        try {
            ...
            $sql = "SELECT ar.PK_id_artista, ar.nombre_artistico, ar.ruta_foto_perfil AS foto_perfil, ar.biografia
                    ...
                    WHERE sa.FK_id_usuario = ? AND sa.activo = 1 AND ar.estado_disponible = 1
    ...
    public function notificarSeguidores($id_artista, $nombre_artista, $titulo_cancion, $id_cancion)
    {
        ...
        try {
            ...
            $sqlSeg = "SELECT FK_id_usuario FROM Seguimiento_Artista WHERE FK_id_artista = ? AND activo = 1";
