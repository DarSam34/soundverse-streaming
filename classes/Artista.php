...
class Artista
{
    ...
    public function obtenerTotalSeguidores($id_artista)
    {
        $this->reconectar();
        try {
            $sql = "SELECT COUNT(*) AS total FROM Seguimiento_Artista WHERE FK_id_artista = ? AND activo = 1";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$id_artista]);
            $res = $stmt->fetch(PDO::FETCH_ASSOC);
            return (int) ($res['total'] ?? 0);
    ...
    /**
     * Lista todos los artistas disponibles ordenados alfabéticamente.
     * Encapsula el SQL que antes estaba en el controlador queries.php (M-05).
     */
    public function listarTodos(): array
    {
        $this->reconectar();
        try {
            $sql = "SELECT PK_id_artista, nombre_artistico, biografia, ruta_foto_perfil, verificado
                    FROM Artista
                    WHERE estado_disponible = 1
            ...
            $this->logError('listarTodos(): ' . $e->getMessage());
            return [];
        } finally {
            $this->db = null;
        }
