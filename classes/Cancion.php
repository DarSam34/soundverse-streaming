...
class Cancion
{
        ...
        try {
            $sql = "SELECT c.PK_id_cancion, c.titulo, c.duracion_segundos, c.ruta_archivo_audio,
                           ...
                           (SELECT COUNT(*) FROM Seguimiento_Artista sa 
                            WHERE sa.FK_id_artista = ar.PK_id_artista AND sa.FK_id_usuario = ? AND sa.activo = 1) AS sigue_artista
                    FROM Cancion c
    ...
    public function eliminarCancion($id)
    {
        ...
        try {
            ...
            // Borrado lógico en tabla pivote transaccional (Detalle_Playlist).
            // Se usa UPDATE activo=0 en lugar de DELETE físico (Regla Oro #4).
            // La canción desaparece de las playlists una vez desactivada.
            $sqlDetalle = "UPDATE Detalle_Playlist SET activo = 0 WHERE FK_id_cancion = ?";
