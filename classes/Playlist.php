...
class Playlist
{
    ...
    public function listarPlaylistsUsuario($id_usuario)
    {
        ...
        try {
            ...
            $sql = "SELECT p.*,
                    ...
                    FROM Playlist p
                    LEFT JOIN Detalle_Playlist dp ON p.PK_id_playlist = dp.FK_id_playlist AND dp.activo = 1
    ...
    public function agregarCancion($id_playlist, $id_cancion, $orden = null)
    {
        ...
        try {
            ...
            $sqlMax = "SELECT COUNT(*) as total FROM Detalle_Playlist WHERE FK_id_playlist = ? AND activo = 1";
            ...
            $sqlCheck = "SELECT COUNT(*) as total FROM Detalle_Playlist WHERE FK_id_playlist = ? AND FK_id_cancion = ? AND activo = 1";
            $stmtCheck = $this->db->prepare($sqlCheck);
            ...
            $sqlOrden = "SELECT COALESCE(MAX(orden_reproduccion), 0) + 1 AS sig_orden FROM Detalle_Playlist WHERE FK_id_playlist = ? AND activo = 1";
    ...
    public function removerCancion($id_playlist, $id_cancion)
    {
        $this->reconectar();
        try {
            // Borrado lógico permitido en tabla pivote transaccional (Detalle_Playlist),
            // se usa UPDATE activo=0 en lugar de DELETE físico (Regla Oro #4).
            $sql = "UPDATE Detalle_Playlist SET activo = 0 WHERE FK_id_playlist = ? AND FK_id_cancion = ?";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$id_playlist, $id_cancion]);
            $eliminado = $stmt->rowCount() > 0;
            if ($eliminado) {
                Utilidades::registrarLog(
                    'operaciones',
    ...
    public function obtenerCancionesPlaylist($id_playlist)
    {
        ...
        try {
            ...
            $sql = "SELECT c.PK_id_cancion, c.titulo, c.duracion_segundos, c.ruta_archivo_audio,
                    ...
                    WHERE dp.FK_id_playlist = ? AND dp.activo = 1 AND c.estado_disponible = 1
    ...
    public function reordenarCancion($id_playlist, $id_cancion, $direccion)
    {
        ...
        try {
            ...
            $sqlOri = "SELECT orden_reproduccion FROM Detalle_Playlist WHERE FK_id_playlist = ? AND FK_id_cancion = ? AND activo = 1";
            ...
            if ($direccion === 'up') {
                $sqlVec = "SELECT FK_id_cancion, orden_reproduccion FROM Detalle_Playlist
                           WHERE FK_id_playlist = ? AND orden_reproduccion < ? AND activo = 1
            ...
            if (!$vecina)
                return false; // Ya está al inicio o al final
    ...
    public function agregarCancionColaborativa($id_playlist, $id_cancion, $id_usuario_solicitante)
    {
        ...
        try {
            ...
            $sqlChk = "SELECT COUNT(*) as total FROM Detalle_Playlist WHERE FK_id_playlist = ? AND FK_id_cancion = ? AND activo = 1";
