...
<div class="container-fluid fade-in">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-list-music me-2 text-primary"></i> <span data-key="pl_titulo">Mis Playlists</span></h2>
        <button class="btn btn-primary rounded-pill px-4 shadow-sm" data-bs-toggle="modal"
            data-bs-target="#modalNuevaPlaylist">
            <i class="fas fa-plus me-1"></i> <span data-key="pl_nueva">Nueva Playlist</span>
        </button>
    </div>

    <div class="row" id="lista-playlists">
        <div class="col-12 text-center py-4">
            <div class="spinner-border text-primary" role="status"></div>
            <p class="mt-2 text-muted" data-key="pl_cargando">Cargando playlists...</p>
        </div>
    </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4><i class="fas fa-globe-americas me-2 text-success"></i> <span data-key="pl_explorar">Explorar Playlists
                Públicas</span></h4>
        <div class="input-group" style="max-width: 300px;">
            <span class="input-group-text bg-light border-0"><i class="fas fa-search text-muted"></i></span>
            <input type="text" id="buscar-playlist-publica" class="form-control border-0 bg-light"
                placeholder="Buscar por nombre..." oninput="buscarPlaylistsPublicas(this.value)">
        </div>
    </div>
    <div class="row" id="lista-playlists-publicas">
        <div class="col-12 text-center py-3">
            <span class="text-muted"><i class="fas fa-search me-1"></i> Escribe para buscar playlists públicas de otros
                usuarios.</span>
        </div>
    </div>
</div>

<div class="modal fade" id="modalNuevaPlaylist" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title"><i class="fas fa-plus-circle me-2"></i> <span data-key="pl_nueva">Nueva
                        Playlist</span></h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4">
                <div class="mb-3">
                    <label class="form-label fw-bold text-muted" data-key="pl_nombre">Nombre de la lista</label>
                    <input type="text" id="playlist-nombre" class="form-control form-control-lg bg-light border-0"
                        placeholder="Ej. Rock Clásico" maxlength="100">
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold text-muted" data-key="pl_visibilidad">Privacidad</label>
                    <select id="playlist-visibilidad" class="form-select form-select-lg bg-light border-0">
                        <option value="Publica" data-key="pl_publica">Pública (Todos pueden verla)</option>
                        <option value="Privada" data-key="pl_privada">Privada (Solo tú)</option>
                        <option value="Colaborativa" data-key="pl_colaborativa">Colaborativa (Pública y otros pueden
                            añadir)</option>
                    </select>
                </div>
            </div>
            <div class="modal-footer border-0">
                <button type="button" class="btn btn-light px-4" data-bs-dismiss="modal"
                    data-key="pl_cancelar">Cancelar</button>
                <button type="button" class="btn btn-primary px-4" id="guardar-playlist">
                    <i class="fas fa-save me-1"></i> <span data-key="pl_guardar">Guardar</span>
                </button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="modalVerPlaylist" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-dark text-white">
                <h5 class="modal-title"><i class="fas fa-music me-2 text-primary"></i> Canciones de la playlist</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-0" id="contenido-playlist">
            </div>
        </div>
    </div>
</div>
...
<script>
                ...
                if (resp.status === 'success') {
                    ...
                    <div class="col-12 text-center my-5">
                        ...
                        resp.data.forEach(function (pl) {
                            let iconoVis = 'fa-globe-americas';
                            let colorVis = 'text-success';

                            if (pl.visibilidad === 'Privada') {
                                iconoVis = 'fa-lock';
                                colorVis = 'text-warning';
                            } else if (pl.visibilidad === 'Colaborativa') {
                                iconoVis = 'fa-users';
                                colorVis = 'text-primary'; // Usamos primary para el tono púrpura/azul de Soundverse
                            }
                            ...
                            <div class="card h-100 shadow-sm border-0 playlist-card bg-white">
                                ...
                                <div class="card-footer bg-transparent border-0 d-flex gap-2 p-3 pt-0">
                                    ...
                                    <button class="btn btn-outline-danger px-3 rounded-pill" title="Eliminar"
                                        data-id="${pl.PK_id_playlist}"
                                        data-nombre="${pl.nombre_playlist.replace(/"/g, '&quot;')}"
                                        onclick="eliminarPlaylist(this.dataset.id, this.dataset.nombre)">
    ...
    // Evento delegado para el botón guardar (funciona incluso si el modal se recarga)
    $(document).on('click', '#guardar-playlist', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                if (resp.status === 'success' || resp.success === true) {
                    ...
                    $('#playlist-nombre').val('');
                    if (typeof cargarPlaylists === 'function') cargarPlaylists();
                } else {
                    ...
                    resp.data.forEach(function (c, idx) {
                        ...
                        <td class="text-end px-3">
                            <button class="btn btn-sm btn-success text-white me-1"
                                data-id="${c.PK_id_cancion}"
                                data-titulo="${c.titulo.replace(/"/g, '&quot;')}"
                                data-artista="${c.artista.replace(/"/g, '&quot;')}"
                                data-audio="../${c.ruta_archivo_audio}"
                                onclick="reproducirCancion(this.dataset.id, this.dataset.titulo, this.dataset.artista, this.dataset.audio)">
    ...
    window.reproducirPlaylistCompleta = function (esPropietario = true) {
        ...
        if (esFreeYAjena) {
            ...
            // DESHABILITAR el botón de shuffle en el reproductor
            if (typeof window.actualizarEstadoBotonShuffle === 'function') {
                window.actualizarEstadoBotonShuffle(true);
            }
            Swal.fire({
                icon: 'info',
            ...
            // Para Premium o playlists propias, habilitar el botón de shuffle normalmente
            if (typeof window.actualizarEstadoBotonShuffle === 'function') {
                window.actualizarEstadoBotonShuffle(false);
            }
        }
...
<div class="col-md-4 col-sm-6 mb-3">
    <div class="card border-0 shadow-sm h-100" style="border-radius:12px; border-left: 4px solid #48bb78 !important;">
        ...
        <div class="card-footer bg-transparent border-0 pt-0 d-flex gap-1">
            <button class="btn btn-outline-info btn-sm rounded-pill flex-fill"
                onclick="verPlaylistPublica(${pl.PK_id_playlist})">
    ...
    window.verPlaylistPublica = function (idPlaylist) {
        $.ajax({
            url: 'php/queries.php?caso=obtener_canciones_playlist',
            type: 'POST',
            data: { id_playlist: idPlaylist },
            ...
            success: function (resp) {
                if (resp.status === 'success') {
                    if (resp.data.length === 0) {
                        Swal.fire('Playlist vacía', 'Esta playlist no tiene canciones.', 'info');
                        return;
                    }

                    // Mostrar modal con las canciones
                    let html = '<div class="table-responsive"><table class="table table-striped">';
                    html += '<thead><tr><th>#</th><th>Título</th><th>Artista</th><th>Duración</th></tr></thead><tbody>';
