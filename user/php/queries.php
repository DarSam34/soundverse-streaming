...
switch ($caso) {
    ...
    case 'listar_todos_artistas':
        ...
        try {
            ...
            $artistas = $artistaObj->listarTodos();
...
function removerCancionPlaylist()
{
    ...
    // Validar que la playlist pertenezca al usuario
    try {
        $db = (new Conexion())->conectar();
        $stmt = $db->prepare("SELECT COUNT(*) as total FROM Playlist WHERE PK_id_playlist = ? AND FK_id_usuario = ? AND estado_disponible = 1");
        $stmt->execute([$id_playlist, $id_usuario]);
        if ($stmt->fetch(PDO::FETCH_ASSOC)['total'] == 0) {
            echo json_encode(['status' => 'error', 'message' => 'No tienes permiso para modificar esta playlist.']);
            $db = null;
            return;
        }
        $db = null;
    } catch (PDOException $e) {
        echo json_encode(['status' => 'error', 'message' => 'Error de validación.']);
        return;
    }
